...
def pullandmerge(key):
    ...
    for i in range(3):
        ...
        for j in range(4):
            csv_name = str((j+1)*0.25) + plane_name
            # 来一个对应位置的存储
            df = pd.DataFrame()
            # 定义缩放因子对的变量和目录
            for num2 in range(len(scale_factor)):
                ...
                print(df_scale[key])
                df.insert(loc=num2+1, value=df_scale[key], allow_duplicates=True, column=str_factors)
            df.insert(loc=0, value=df_scale['V28'], allow_duplicates=True, column='V28')
            df.to_excel(dir_post+nozzle+'\\48\\'+csv_name.replace('.csv', '.xlsx'))
