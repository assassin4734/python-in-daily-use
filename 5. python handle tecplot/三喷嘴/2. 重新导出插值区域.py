...
def tecexport(dir_lay_f, dir_dst_f, dofx):
...
if __name__ == "__main__":
    ...
    # distance_folder = ['2250']
    sw_folder = ['388', '438', '508']
    scale_factor = [1, 0.9, 0.8, 0.7, 0.6, 0.45, 0.4, 0.35, 0.3, 0.25]
    file = ['0.25d-vorticity.lay', '0.5d-vorticity.lay', '0.75d-vorticity.lay', '1d-vorticity.lay']
    print('# 找到工作目录')
    for distance in distance_folder:
        ...
        for sw in sw_folder:    # 变间距目录
            ...
            for num in range(len(scale_factor)):
                ...
                for lay in file:
                    dir_dst = dir_post_working + lay
                    # 
                    print('# 导出插值区域')
                    # 
                    dir_dat = (dir_post_working+lay).strip('-vorticity.lay')+'.dat'
                    try:
                        tecexport(dir_dst, dir_dat, x_d)
                    except:
                        print('文件不存在')
                        fail.append(dir_dst)
print('all done')
