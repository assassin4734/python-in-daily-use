    ...
    for i in range(len(dir_files)):
        ...
        if i != 2:
            tp.macro.execute_extended_command(command_processor_id='Extract Precise Line',
                                              command='XSTART = 1 YSTART = 0.1 ZSTART = 0 XEND = 1 YEND = 3.5 ZEND = 0 NUMPTS = 400 EXTRACTTHROUGHVOLUME = F EXTRACTTOFILE = F ')
        else:
            tp.macro.execute_extended_command(command_processor_id='Extract Precise Line',
                                              command='XSTART = 0 YSTART = 0.1 ZSTART = 0 XEND = 0 YEND = 3.5 ZEND = 0 NUMPTS = 400 EXTRACTTHROUGHVOLUME = F EXTRACTTOFILE = F ')
        tp.macro.execute_command('$!RedrawAll')
        dir_zone = dir_post_scale + "相互作用线-" + lay[-11:-4] + ".csv"
        command_e_as_f = 'VarNames:FrOp=1:ZnCount=1:ZnList=[2]:AllVars:ValSep=",":FNAME="'+dir_zone+'"'
