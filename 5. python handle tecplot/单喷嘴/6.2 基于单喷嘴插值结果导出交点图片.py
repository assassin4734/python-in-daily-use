...
print('定位后处理的目录')
# onenozzlefolder = {'变当量比': 'eq\\postprocessing-correction\\',
#                    '变旋流数': 'different swirl number\\postprocessing-correction\\'}
onenozzlefolder = {'变当量比': 'eq\\postprocessing-correction\\',}
# folder = {'sw_folder': ["z-28.5", "z-35.5", "z-40.5", "z-45.5", "z-52.5"],
#           'eq_folder': ["eq=0.55", "eq=0.65", "eq=0.75", "eq=0.85", "eq=0.95"]}
folder = {'eq_folder': ["eq=0.55", "eq=0.65", "eq=0.75", "eq=0.85", "eq=0.95"]}
...
for para in onenozzlefolder:
    ...
    for var in folder[fol]:
        ...
        dir_pod = dir_para + var + '\\pod_analyse\\pod_reconstructs\\'
        file_dat = []
        ...
        for scale in scale_num:
            ...
            with open(lay_txt, 'r+', encoding='utf-8') as f:
                content = f.read()
                content = content.replace(txt_src, str(scale) + '-velocity.dat')
                f.seek(0, 0)
                f.write(content)
                f.close()
