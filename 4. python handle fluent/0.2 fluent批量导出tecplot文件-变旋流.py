...
def log_open():
    ...
    '''
    file_data = ""
    ori_log = open("G:\\Assassin\\1nozzle\\data export journal.txt", "r", encoding="utf-8")
    for line in ori_log:
        file_data += line
    ori_log.close()
    return(file_data)
...
for files in file_folder:
    ...
    for num in range(len(scale_factor)):
        ...
        dir_fluent_scale = dir_fluent + '\\' + str(files) + '-' + str_factors
        # 日志的保存路径
        journal_place = dir_post + '\\' + str(files) + '-' + str_factors + '\\' + str(files) + '-' + str_factors + '.txt'
        # plt的保存路径
        tecplot_place = dir_post + '\\' + str(files) + '-' + str_factors + '\\28.5-1'
